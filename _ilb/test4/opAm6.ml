open Imlb
let range a b =
  a + (Random.int (b - a))

let sq_dist (x1, y1) (x2, y2) =
  let dx = x2 - x1 in
  let dy = y2 - y1 in
  let sq_dist = (dx * dx) + (dy * dy) in
  (sq_dist)

let v_next v =
  match Random.int 3 with
  | 0 -> v
  | 1 -> v + 1
  | 2 -> v - 1
  | _ -> assert false
type f3 =
  | F1
  | F2
  | F3
  | F7
  | F9
type f4 =
  | F4
  | F5
type i4 =
  | Il of int list
  | In of int
  | Iv of int
  | Ir of int * int
let l_next l =
  match l with
  | Il (h::t) -> Il (List.rev (h::(List.rev t)))
  | Il [] -> assert false
  | In v -> In v
  | Iv v -> Iv (v_next v)
  | Ir _ -> assert false
let get_v l =
  match l with
  | Il (h::_) -> h
  | Il [] -> assert false
  | In v -> v
  | Iv v -> v
  | Ir _ -> assert false
type env3 =
  | P3 of p3
  | P1 of img
type c = (int * int * int)
type op2 =
  | Img_t of ((int * int) * int)
  | Lin_t of (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Tri_t of (i4 * i4) * (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Sirc_t of (i4 * i4) * i4 * c * f3 * f4
  | Rest_u of (i4 * i4 * i4 * i4) * c
  | Rest_t of (i4 * i4 * i4 * i4) * c * f3 * f4
  | Rect_u of (i4 * i4 * i4 * i4) * c
  | Rect_t of (i4 * i4 * i4 * i4) * c * f3 * f4
  | Quad_t of (i4 * i4) * (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Circ_u of (i4 * i4) * i4 * c
  | Circ_t of (i4 * i4) * i4 * c * f3 * f4
  | Px_t of (i4 * i4) * c * f3 * f4
type op4 =
  | Img_r of ((int * int) * int)
  | Lin_r of (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Tri_r of (i4 * i4) * (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Sirc_r of (i4 * i4) * i4 * c * f3 * f4
  | Rest_s of (i4 * i4 * i4 * i4) * c
  | Rest_r of (i4 * i4 * i4 * i4) * c * f3 * f4
  | Rect_s of (i4 * i4 * i4 * i4) * c
  | Rect_r of (i4 * i4 * i4 * i4) * c * f3 * f4
  | Quad_r of (i4 * i4) * (i4 * i4) * (i4 * i4) * c * f3 * f4
  | Circ_s of (i4 * i4) * i4 * c
  | Circ_r of (i4 * i4) * i4 * c * f3 * f4
  | Px_r of (i4 * i4) * c * f3 * f4

type lay2 =
  | Layer_t of append_mode * (int * int * int) array * op2 list
  | Base_t of (int * int) * op2 list

type lay4 =
  | Layer of append_mode * (int * int * int) array * op4 list
  | Base of (int * int) * op4 list

let init_1 v =
  match v with
  | Il l -> Il l
  | In v -> In v
  | Iv v -> Iv v
  | Ir (a, b) -> In (range a b)
let f_aff f rs =
  match f with
  | F1 -> List.filter (fun (_, _) -> (Random.int 2) = 0) rs
  | F2 -> List.map (fun (x, y) -> (v_next x, v_next y)) rs
  | F3 -> List.filter (fun (_, _) -> (Random.int 19) <> 0) rs
  | F7 -> List.filter (fun (_, _) -> (Random.int 11) <> 0) rs
  | F9 -> List.filter (fun (_, _) -> (Random.int 7) = 0) rs
let f_next a (x, y) =
  match a with
  | F4 -> (l_next x, l_next y)
  | F5 -> (x, y)
let c0 c =
  match c with
  | (c0, _, _) -> (c0)

let op_an_f d op_l =
  let get_lay lay =
    match lay with
    | lay0::_ -> (lay0)
    | [] -> failwith "get_lay: not-found"
  in
  let rec get_img env =
    match env with
    | ("img", P3 r) :: env -> (P3 r)
    | ("img", P1 r) :: env -> (P1 r)
    | _ :: env -> get_img env
    | [] -> failwith "get_img: not-found"
  in
  let rec aux3 op_t acc =
    match op_t with
    | Img_t ((w, h), v) :: op_t ->
        let acc = Img_t ((w, h), v) :: acc in
        aux3 op_t acc
    | Px_t ((x, y), v, f, a) :: op_t ->
        let x, y = f_next a (x, y) in
        let this = Px_t ((x, y), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Lin_t ((x1, y1), (x2, y2), v, f, a) :: op_t ->
        let x1, y1 = f_next a (x1, y1) in
        let x2, y2 = f_next a (x2, y2) in
        let this = Lin_t ((x1, y1), (x2, y2), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Tri_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: op_t ->
        let x1, y1 = f_next a (x1, y1) in
        let x2, y2 = f_next a (x2, y2) in
        let x3, y3 = f_next a (x3, y3) in
        let this = Tri_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Sirc_t ((cx, cy), (r), v, f, a) :: op_t ->
        let cx, cy = f_next a (cx, cy) in
        let r, _ = f_next a (r, r) in
        let this = Sirc_t ((cx, cy), (r), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Rest_t ((x, y, w, h), v, f, a) :: op_t ->
        let x, y = f_next a (x, y) in
        let w, h = f_next a (w, h) in
        let this = Rest_t ((x, y, w, h), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Rest_u ((x, y, w, h), v) :: op_t ->
        let x = l_next (x) in
        let y = l_next (y) in
        let w = l_next (w) in
        let h = l_next (h) in
        let this = Rest_u ((x, y, w, h), v) in
        let acc = this :: acc in
        aux3 op_t acc
    | Rect_t ((x, y, w, h), v, f, a) :: op_t ->
        let x, y = f_next a (x, y) in
        let w, h = f_next a (w, h) in
        let x = l_next (x) in
        let y = l_next (y) in
        let w = l_next (w) in
        let h = l_next (h) in
        let this = Rect_t ((x, y, w, h), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Rect_u ((x, y, w, h), v) :: op_t ->
        let x = l_next (x) in
        let y = l_next (y) in
        let w = l_next (w) in
        let h = l_next (h) in
        let this = Rect_u ((x, y, w, h), v) in
        let acc = this :: acc in
        aux3 op_t acc
    | Circ_u ((cx, cy), r, v) :: op_t ->
        let cx = l_next (cx) in
        let cy = l_next (cy) in
        let r = l_next (r) in
        let this = Circ_u ((cx, cy), r, v) in
        let acc = this :: acc in
        aux3 op_t acc
    | Circ_t ((cx, cy), (r), v, f, a) :: op_t ->
        let cx, cy = f_next a (cx, cy) in
        let r, _ = f_next a (r, r) in
        let this = Circ_t ((cx, cy), (r), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | Quad_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: op_t ->
        let x1, y1 = f_next a (x1, y1) in
        let x2, y2 = f_next a (x2, y2) in
        let x3, y3 = f_next a (x3, y3) in
        let this = Quad_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) in
        let acc = this :: acc in
        aux3 op_t acc
    | [] -> (List.rev acc)
  in
  let rec aux2 d op_t env _op_t =
    (*
    if d <= 0 then () else
    *)
    match op_t with
    | Img_t ((w, h), v) :: op_t ->
        let env =
          match v with
          | 3 ->
              let r = create_p3 120 90 in
              let env = ("img", (P3 r)) :: env in
              (env)
          | 1 ->
              let r = create_img 120 90 in
              let env = ("img", (P1 r)) :: env in
              (env)
          | _ ->
              assert false
        in
        aux2 d op_t env _op_t
    | Px_t ((x, y), v, f, _) :: op_t ->
        let x = get_v x in
        let y = get_v y in
        let rs = [(x, y)] in
        let p = get_img env in
        let rs = f_aff f rs in
        List.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Lin_t ((x1, y1), (x2, y2), v, f, _) :: op_t ->
        let x1 = get_v x1 in
        let y1 = get_v y1 in
        let x2 = get_v x2 in
        let y2 = get_v y2 in
        let rs = draw_line_r (x1, y1) (x2, y2) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Tri_t ((x1, y1), (x2, y2), (x3, y3), v, f, _) :: op_t ->
        let x1 = get_v x1 in
        let y1 = get_v y1 in
        let x2 = get_v x2 in
        let y2 = get_v y2 in
        let x3 = get_v x3 in
        let y3 = get_v y3 in
        let rs = draw_tri_r (x1, y1) (x2, y2) (x3, y3) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Sirc_t ((cx, cy), (r), v, f, _) :: op_t ->
        let cx = get_v cx in
        let cy = get_v cy in
        let r = get_v r in
        let rs = strk_circ_r (cx, cy) (r) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Rest_t ((x, y, w, h), v, f, _) :: op_t ->
        let x = get_v x in
        let y = get_v y in
        let w = get_v w in
        let h = get_v h in
        let rs = strk_rect_r (x, y, w, h) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Rect_t ((x, y, w, h), v, f, _) :: op_t ->
        let x = get_v x in
        let y = get_v y in
        let w = get_v w in
        let h = get_v h in
        let rs = draw_rect_r (x, y, w, h) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Rest_u ((x, y, w, h), v) :: op_t ->
        let x = get_v x in
        let y = get_v y in
        let w = get_v w in
        let h = get_v h in
        let p = get_img env in
        for _x = x to pred (x + w) do
          match p with
          | P1 r -> put_px r (_x, y) (c0 v);
          | P3 r -> put_p3_rgb r _x y v;
        done;
        let _y = pred (y + h) in
        for _x = x to pred (x + w) do
          match p with
          | P1 r -> put_px r (_x, _y) (c0 v);
          | P3 r -> put_p3_rgb r _x _y v;
        done;
        for _y = y to pred (y + h) do
          match p with
          | P1 r -> put_px r (x, _y) (c0 v);
          | P3 r -> put_p3_rgb r x _y v;
        done;
        let _x = pred (x + w) in
        for _y = y to pred (y + h) do
          match p with
          | P1 r -> put_px r (_x, _y) (c0 v);
          | P3 r -> put_p3_rgb r _x _y v;
        done;
        aux2 d op_t env _op_t
    | Rect_u ((x, y, w, h), v) :: op_t ->
        let x = get_v x in
        let y = get_v y in
        let w = get_v w in
        let h = get_v h in
        let p = get_img env in
        for _y = y to pred (y + h) do
          for _x = x to pred (x + w) do
            match p with
            | P1 r -> put_px r (_x, _y) (c0 v);
            | P3 r -> put_p3_rgb r _x _y v;
          done;
        done;
        aux2 d op_t env _op_t
    | Circ_u ((cx, cy), _r, v) :: op_t ->
        let cx = get_v cx in
        let cy = get_v cy in
        let _r = get_v _r in
        let _p = get_img env in
        for _y = cy - _r to cy + _r do
          for _x = cx - _r to cx + _r do
            let p = (_x, _y) in
            let d = sq_dist (cx, cy) (p) in
            if d < (_r * _r) then begin
              match _p with
              | P1 r -> put_px r (_x, _y) (c0 v);
              | P3 r -> put_p3_rgb r _x _y v;
            end;
          done;
        done;
        aux2 d op_t env _op_t
    | Circ_t ((cx, cy), (r), v, f, _) :: op_t ->
        let cx = get_v cx in
        let cy = get_v cy in
        let r = get_v r in
        let rs = draw_circ_r (cx, cy) (r) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | Quad_t ((x1, y1), (x2, y2), (x3, y3), v, f, _) :: op_t ->
        let x1 = get_v x1 in
        let y1 = get_v y1 in
        let x2 = get_v x2 in
        let y2 = get_v y2 in
        let x3 = get_v x3 in
        let y3 = get_v y3 in
        let rs = quad_bcurve_r (x1, y1) (x2, y2) (x3, y3) in
        let p = get_img env in
        let rs = Array.to_list rs in
        let rs = f_aff f rs in
        let rs = Array.of_list rs in
        Array.iter (fun (x, y) ->
          match p with
          | P1 r -> put_px r (x, y) (c0 v);
          | P3 r -> put_p3_rgb r x y v;
        ) rs;
        aux2 d op_t env _op_t
    | [] ->
        (*
        let r = get_img env in
        print_p3 r;
        free_p3 r;
        *)
        (*
        let _op_t = aux3 _op_t [] in
        aux2 (pred d) _op_t [] _op_t
        *)
        (env)
  in
  let rec aux d op_r acc =
    match op_r with
    | Img_r ((w, h), v) :: op_r ->
        let acc = Img_t ((w, h), v) :: acc in
        aux d op_r acc
    | Px_r ((x, y), v, f, a) :: op_r ->
        let x = init_1 x in
        let y = init_1 y in
        let acc = Px_t ((x, y), v, f, a) :: acc in
        aux d op_r acc
    | Lin_r ((x1, y1), (x2, y2), v, f, a) :: op_r ->
        let x1 = init_1 x1 in
        let y1 = init_1 y1 in
        let x2 = init_1 x2 in
        let y2 = init_1 y2 in
        let acc = Lin_t ((x1, y1), (x2, y2), v, f, a) :: acc in
        aux d op_r acc
    | Tri_r ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: op_r ->
        let x1 = init_1 x1 in
        let y1 = init_1 y1 in
        let x2 = init_1 x2 in
        let y2 = init_1 y2 in
        let x3 = init_1 x3 in
        let y3 = init_1 y3 in
        let acc = Tri_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: acc in
        aux d op_r acc
    | Sirc_r ((cx, cy), (r), v, f, a) :: op_r ->
        let cx = init_1 cx in
        let cy = init_1 cy in
        let r = init_1 r in
        let acc = Sirc_t ((cx, cy), (r), v, f, a) :: acc in
        aux d op_r acc
    | Rect_s ((x, y, w, h), v) :: op_r ->
        let x = init_1 x in
        let y = init_1 y in
        let w = init_1 w in
        let h = init_1 h in
        let acc = Rect_u ((x, y, w, h), v) :: acc in
        aux d op_r acc
    | Rect_r ((x, y, w, h), v, f, a) :: op_r ->
        let x = init_1 x in
        let y = init_1 y in
        let w = init_1 w in
        let h = init_1 h in
        let acc = Rect_t ((x, y, w, h), v, f, a) :: acc in
        aux d op_r acc
    | Circ_s ((cx, cy), r, v) :: op_r ->
        let cx = init_1 cx in
        let cy = init_1 cy in
        let r = init_1 r in
        let acc = Circ_u ((cx, cy), r, v) :: acc in
        aux d op_r acc
    | Circ_r ((cx, cy), r, v, f, a) :: op_r ->
        let cx = init_1 cx in
        let cy = init_1 cy in
        let r = init_1 r in
        let acc = Circ_t ((cx, cy), r, v, f, a) :: acc in
        aux d op_r acc
    | Quad_r ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: op_r ->
        let x1 = init_1 x1 in
        let y1 = init_1 y1 in
        let x2 = init_1 x2 in
        let y2 = init_1 y2 in
        let x3 = init_1 x3 in
        let y3 = init_1 y3 in
        let acc = Quad_t ((x1, y1), (x2, y2), (x3, y3), v, f, a) :: acc in
        aux d op_r acc
    | Rest_s ((x, y, w, h), v) :: op_r ->
        let x = init_1 x in
        let y = init_1 y in
        let w = init_1 w in
        let h = init_1 h in
        let acc = Rest_u ((x, y, w, h), v) :: acc in
        aux d op_r acc
    | Rest_r ((x, y, w, h), v, f, a) :: op_r ->
        let x = init_1 x in
        let y = init_1 y in
        let w = init_1 w in
        let h = init_1 h in
        let acc = Rest_t ((x, y, w, h), v, f, a) :: acc in
        aux d op_r acc
    | [] ->
        let op_t = (List.rev acc) in
        (op_t)
        (*
        let env = aux2 d op_t [] op_t in
        (env)
        *)
  in
  let rec aux0 op_l acc =
    match op_l with
    | Layer (append_mode, cs, op_r) :: op_l ->
        let this = Layer_t (append_mode, cs, aux 0 op_r []) in
        let acc = this :: acc in
        aux0 op_l acc
    | Base ((w, h), op_r) :: op_l ->
        let this = Base_t ((w, h), aux 0 op_r []) in
        let acc = this :: acc in
        aux0 op_l acc
    | [] ->
        (List.rev acc)
  in
  let rec auxn op_l acc =
    match op_l with
    | Layer_t (append_mode, cs, op_r) :: op_l ->
        let op_r = aux3 op_r [] in
        let acc = Layer_t (append_mode, cs, op_r) :: acc in
        auxn op_l acc
    | Base_t ((w, h), op_r) :: op_l ->
        let op_r = aux3 op_r [] in
        let acc = Base_t ((w, h), op_r) :: acc in
        auxn op_l acc
    | [] ->
        (List.rev acc)
  in
  let rec aux1 d op_l _op_l env lay =
    match op_l with
    | Layer_t (append_mode, cs, op_r) :: op_l ->
        let env = aux2 d op_r [] op_r in
        let p = get_img env in
        let l0 = get_lay lay in
        begin
          match (l0, p) with
          | P3 r1, P1 r2 -> p3_lay_p1 r1 r2 append_mode cs
          | _, _ -> failwith "lay"
        end;
        begin match (l0, p) with
          | P3 _, P1 r2 -> free_img r2
          | _, _ -> failwith "free"
        end;
        aux1 d op_l _op_l env (lay)
        (*
        lay
        *)
    | Base_t ((w, h), op_r) :: op_l ->
        let env = aux2 d op_r [] op_r in
        (*
        let r, p = get_img env in
        *)
        let p = get_img env in
        aux1 d op_l _op_l env (p::lay)
        (*
        create
        *)
    | [] ->
        begin
          (*
          let p = get_img env in
          match p with
          *)
          let l0 = get_lay lay in
          match l0 with
          | P1 r ->
              print_img r;
              free_img r;
          | P3 r ->
              print_p3 r;
              free_p3 r;
        end;
        if d > 0 then begin
          let op_l = auxn _op_l [] in
          aux1 (pred d) op_l op_l [] []
        end;
        (*
        print
        free
        *)
        (*
        anim
        *)
  in
  let op_l = aux0 op_l [] in
  aux1 d op_l op_l [] []
