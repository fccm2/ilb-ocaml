...
type op2 =
  | Img_t of (int * int)
  | Lin_t of (int * int) * (int * int) * int * f3 * f4
  | Tri_t of (int * int) * (int * int) * (int * int) * int * f3 * f4
  | Sirc_t of (int * int) * int * int * f3 * f4
  | Rect_u of (int * int * int * int) * int
  | Rect_t of (int * int * int * int) * int * f3 * f4
  | Circ_u of (int * int) * int * int
  | Circ_t of (int * int) * int * int * f3 * f4
  | Px_t of (int * int) * int * f3 * f4
type op3 =
  | Img_r of (int * int)
...
let f_aff f rs =
  match f with
  | F0 -> List.filter (fun (_, _) -> (Random.int 6) = 0) rs
...
let op_an_f d op_r =
  ...
  let rec aux2 d op_t env _op_t =
    ...
    | Rect_u ((x, y, w, h), v) :: op_t ->
        let r = get_img env in
        draw_rect r (x, y, w, h) v;
        aux2 d op_t env _op_t
    | Circ_u ((cx, cy), _r, v) :: op_t ->
        let r = get_img env in
        draw_circ r (cx, cy) _r v;
        aux2 d op_t env _op_t
